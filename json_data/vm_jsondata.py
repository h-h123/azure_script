...
def check_unsecured_vm_instances(subscription_ids):
    ...
    csv_file_path3 = 'New_report_VM.csv'
    json_path = "VM_data.json"
    # data dictionary for the current subscription
    details_dict = {
    "Subscription Name": "",
    "Subscription ID": "",
    "Total VMs checked": 0,
    ...
    try:
        ...
        for subscription_id in subscription_ids:
            ...
            for vm in vms:
                ...
                sentences2.append(f"\nChecking for VM '{vm.name}' in the Resource group '{resource_group_of_vm}'...")
                sentences3.append(f"\nChecking for VM '{vm.name}' in the Resource group '{resource_group_of_vm}'...")
                details_dict["Details"][f"Resource_Group of VM '{vm.name}'"] = resource_group_of_vm
                ...
                if has_public_ip_and_open_to_all and overly_permissive_nsg_rule and misconfigured_security_rule:
                    ...
                    details_dict["Details"][f"Vulnerability found in VM `{vm.name}`"] = "Reason - It has overly-permissive inbound rules for management ports in Network Security Group of VM and Open remote management ports are exposing VM to a high level of risk from Internet-based attack."
        ...
        ####################################################################################
                ...
                for disk in disks_list:
                    ...
                    sentences2.append(f"\nChecking for Disk '{disk.name}' in RG - {RG_of_disk}...")
                    sentences3.append(f"\nChecking for Disk '{disk.name}' in RG - {RG_of_disk}...")
                    details_dict["Details"][f"Resource_Group of Disk `{disk.name}`"] = RG_of_disk
                    ...
                    #check if Disk is attached or not
                    if disk.disk_state == 'Attached':
                        ...
                        details_dict["Details"][f"Disk `{disk.name}` is Attached to VM" ] = disk_attached_vm_name
                    else:
                        ...
                        details_dict["Details"][f"Warning Disk `{disk.name}` is not attached"] = f"RG of Disk `{RG_of_disk}`"
                    ...
                    if not disk.encryption:
                        ...
                        details_dict["Details"][f"Vulnerability in Encryption Setting of Disk `{disk.name}`"] = disk.name 
                    else:
                        ...
                        details_dict["Details"][f"Encryption Setting is configured of Disk `{disk.name}`"] = disk.name 
                    ...
                    if disk.data_access_auth_mode is None:
                        ...
                        details_dict["Details"][f"Vulnerability in Data Access Authentication Mode of Disk `{disk.name}`"] = disk.name                      
                    else:
                        ...
                        details_dict["Details"][f"Data Access Authentication Mode Configured of Disk `{disk.name}`"] = disk.name                      
                    ...
                    if disk.optimized_for_frequent_attach and 'sensitive_data' in disk.tags:
                        ...
                        details_dict["Details"][f"Warning frequent attachment optimization for disk `{disk.name}`"] = disk.name
                    else:
                        ...
                        details_dict["Details"][f"Frequent attachment optimization is not configured for disk `{disk.name}`"] = disk.name
                    ...
                    if disk.bursting_enabled_time and disk.bursting_enabled_time.startswith("peak_hours"):
                        ...
                        details_dict["Details"][f"Warning Bursting is currently enabled during peak operational hours for the disk `{disk.name}`"] =  disk.name
                    else:
                        ...
                        details_dict["Details"][f"Bursting is not enabled for the disk `{disk.name}`"] =  disk.name
            ...
            for sub in subscriptions:
                if sub.subscription_id == subscription_id:
                    ...
                    details_dict["Total VMs checked"] = total_vm_checked_count
            ...
            details_dict["Details"]['Error in checking virtual machines vulnerabilities'] = e
